...
static void aspeed_ast2400_soc_realize(DeviceState *dev, Error **errp)
{
    ...
    aspeed_mmio_map(s->memory, SYS_BUS_DEVICE(&s->uhci), 0,
                    sc->memmap[ASPEED_DEV_UHCI]);
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->uhci), 0,
                       aspeed_soc_ast2400_get_irq(s, ASPEED_DEV_UHCI));
    ...
    aspeed_mmio_map(s->memory, SYS_BUS_DEVICE(&s->gfx), 0,
                    sc->memmap[ASPEED_DEV_GFX]);
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->gfx), 0,
                       aspeed_soc_ast2400_get_irq(s, ASPEED_DEV_GFX));
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->pwm), 0,
                       aspeed_soc_ast2400_get_irq(s, ASPEED_DEV_PWM));
}
