...
#define  XDMA_IRQ_ENG_CTRL_W_MASK  0xBFEFF07F /* TODO */
                              ...
                              unsigned int size)
{
    ...
    if (addr == axc->cmdq_endp) {
        ...
        if (xdma->bmc_cmdq_readp_set) {
            ...
            xdma->regs[TO_REG(axc->eng_stat)] |= axc->eng_complete;

            if (xdma->regs[TO_REG(axc->eng_ctrl)] & axc->eng_complete) {
    ...
    } else if (addr == axc->eng_ctrl) {
        xdma->regs[TO_REG(addr)] = val32 & XDMA_IRQ_ENG_CTRL_W_MASK;
    } else if (addr == axc->eng_stat) {
        trace_aspeed_xdma_write(addr, val);

        idx = TO_REG(addr);
        if (val32 & axc->eng_complete) {
            xdma->regs[idx] &= ~axc->eng_complete;
...
static void aspeed_xdma_reset(DeviceState *dev)
{
    ...
    xdma->regs[TO_REG(axc->eng_stat)] = XDMA_IRQ_ENG_STAT_RESET;
...
static void aspeed_2600_xdma_class_init(ObjectClass *klass, void *data)
{
    ...
    axc->eng_ctrl = XDMA_AST2600_CTRL;
    axc->eng_stat = XDMA_AST2600_STATUS;
    axc->eng_complete = BIT(16) | BIT(17);
}
...
static void aspeed_2500_xdma_class_init(ObjectClass *klass, void *data)
{
    ...
    axc->eng_ctrl = XDMA_IRQ_ENG_CTRL;
    axc->eng_stat = XDMA_IRQ_ENG_STAT;
    axc->eng_complete = BIT(4) | BIT(5);
};
