...
static bool igb_vf_offset_valid(uint16_t vfn, uint32_t offset,
                                uint32_t value,
                                const uint32_t *whitelist, int count)
{
    int i;

    for (i = 0; i < count; i++) {
        if (offset == whitelist[i]) {
            return true;
        }
    }

    /* RA bank: 16 entries × 2 words (RAL + RAH) */
    if (offset >= RA && offset < RA + 32) {
        if ((offset - RA) % 2 == 1) {
            return !!(value & (E1000_RAH_POOL_1 << vfn));
        }
        return true;
    }

    /* RA2 bank: 8 entries × 2 words */
    if (offset >= RA2 && offset < RA2 + 16) {
        if ((offset - RA2) % 2 == 1) {
            return !!(value & (E1000_RAH_POOL_1 << vfn));
        }
        return true;
                                  ...
                                  const void *buf, size_t size)
{
    ...
    uint32_t valid_offsets[IGB_VF_MAX_REGS];
    int num_valid;
    int i;
    int q0 = s->vfn;
    int q1 = s->vfn + IGB_NUM_VM_POOLS;

    igb_vf_reg_list(s->vfn, valid_offsets, &num_valid);

#define NEED_WORDS(n) do { \
    const uint32_t *end = (const uint32_t *)((const uint8_t *)buf + size); \
    if (end - p < (n)) return -IGB_MIG_ERR_BAD_SIZE; \
} while (0)
    ...
    /*
     * TODO: check VFN during pre-copy so mismatches fail early,
     * before the source VM is stopped.
     */
    if (saved_vfn != s->vfn) {
    ...
    NEED_WORDS(num_regs * 2);
    for (i = 0; i < num_regs; i++) {
        uint32_t offset = le32_to_cpu(*p++);
        uint32_t value = le32_to_cpu(*p++);

        if (!igb_vf_offset_valid(s->vfn, offset, value,
                                 valid_offsets, num_valid)) {
            return -IGB_MIG_ERR_BAD_SIZE;
        }
    ...
    NEED_WORDS(2 * sizeof(struct igb_tx) / sizeof(uint32_t));
...
static uint32_t igb_core_vf_dirty_query(IgbVfState *s,
...
static void igb_core_vf_dirty_query_commit(IgbVfState *s,
                                           uint64_t range_iova,
                                           uint64_t range_size)
{
    IGBCore *core = igbvf_get_core(s);
    ...
    for (i = 0; i < ds->num_ranges; i++) {
        ...
        if (start_page < r->nbits) {
            ...
            n = MIN(range_pages, avail);
            uint32_t cleared = bitmap_count_one_with_offset(r->bitmap,
                                                            start_page, n);
...
static bool igb_core_vf_dirty_range_valid(IGBVfDirtyState *ds,
                                          uint64_t range_iova,
                                          uint64_t range_size)
{
    if (!range_size) {
...
static void igbvf_mig_dirty_query_complete(IgbVfState *s)
{
    PCIDevice *dev = pcie_sriov_get_pf(PCI_DEVICE(s));

    stl_le_pci_dma(dev,
                   s->mig.mig_dirty_buf_addr +
                   offsetof(struct igb_mig_dirty_query, status),
                   IGB_MIG_DIRTY_STATUS_COMPLETE,
                   MEMTXATTRS_UNSPECIFIED);
}

static uint32_t igbvf_mig_dirty_query(IgbVfState *s, uint64_t pgsize)
{
    IgbVfMigState *ms = &s->mig;
    IgbVfMigStats *stats = &igbvf_get_core(s)->vf_mig_stats[s->vfn];
    ...
    uint32_t bmp_bytes, status, dirty_pages;
    size_t out_size;
    g_autofree void *bitmap = NULL;

    if (!igb_core_vf_dirty_enabled(s)) {
        return IGB_MIG_DIRTY_STATUS_NOT_ENABLED;
    }

    if (!buf_addr) {
        return IGB_MIG_DIRTY_STATUS_NO_BUFFER;
    }
    ...
    if (!igb_core_vf_dirty_range_valid(ds, range_iova, range_size)) {
        return IGB_MIG_DIRTY_STATUS_BAD_RANGE;
    }

    bmp_bytes = BITS_TO_LONGS(range_size / pgsize) * sizeof(unsigned long);
    bitmap = g_malloc0(bmp_bytes);

    status = igb_core_vf_dirty_query(s, bitmap, bmp_bytes, &out_size,
    ...
    if (out_size) {
        if (pci_dma_write(dev,
            ...
            qemu_log_mask(LOG_GUEST_ERROR,
                ...
                s->vfn, buf_addr);
            return IGB_MIG_DIRTY_STATUS_DMA_FAILED;
        }
    }

    igb_core_vf_dirty_query_commit(s, range_iova, range_size);

    dirty_pages = igbvf_mig_dirty_count(bitmap, out_size);

    stl_le_pci_dma(dev,
                   buf_addr + offsetof(struct igb_mig_dirty_query, bitmap_size),
                   out_size, MEMTXATTRS_UNSPECIFIED);
    stl_le_pci_dma(dev,
...
static void igbvf_mig_dirty_ctrl(IgbVfState *s, uint32_t val)
{
    ...
    case IGB_MIG_DIRTY_CTRL_QUERY:
        ms->mig_dirty_status = igbvf_mig_dirty_query(s, pgsize);
        igbvf_mig_dirty_query_complete(s);
        break;
    }
}
                             ...
                             unsigned size)
{
    ...
    case IGB_MIG_DIRTY_RANGE_IOVA_HI:
        ...
        break;
    case IGB_MIG_DIRTY_RANGE_SIZE_LO:
        ms->mig_dirty_range_size =
            deposit64(ms->mig_dirty_range_size, 0, 32, val);
        break;
    case IGB_MIG_DIRTY_RANGE_SIZE_HI:
        ms->mig_dirty_range_size =
            deposit64(ms->mig_dirty_range_size, 32, 32, val);
        break;
    case IGB_MIG_DIRTY_BUF_ADDR_LO:
