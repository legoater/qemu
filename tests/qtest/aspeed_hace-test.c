                        ...
                        const uint32_t src_addr)
{
    ...
    write_regs(s, base, src_addr,
               sizeof(test_vector_sg1) + sizeof(test_vector_sg2) + sizeof(test_vector_sg3),
                           ...
                           const struct masks *expected)
{
    ...
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_SRC), ==, expected->src_direct);
                           ...
                           const struct masks *expected)
{
    ...
    /*
     *  Check the masks used on Scatter-Gatter command
     */

    /*  Check that everything is pristine zero first */
    g_assert_cmphex(qtest_readl(s, base + HACE_CMD), ==, 0);
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_SRC), ==, 0);
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_DIGEST), ==, 0);
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_DATA_LEN), ==, 0);

    /*  Write src, digest and len registers with 1 */
    qtest_writel(s, base + HACE_HASH_SRC, 0xffffffff);
    qtest_writel(s, base + HACE_HASH_DIGEST, 0xffffffff);
    qtest_writel(s, base + HACE_HASH_DATA_LEN, 0xffffffff);

    /* write SG command */
    qtest_writel(s, base + HACE_CMD, HACE_SG_EN);

    /* Check that the address masking is correct */
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_SRC), ==, expected->src_sg);
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_DIGEST), ==, expected->src_sg);
    g_assert_cmphex(qtest_readl(s, base + HACE_HASH_DATA_LEN), ==, expected->src_sg);
