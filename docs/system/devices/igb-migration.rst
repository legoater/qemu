...
the following register layout::
  ...
  0x030   DIRTY_RANGE_SIZE_LO WO      Low 32 bits of tracked range size
  0x034   DIRTY_RANGE_SIZE_HI WO      High 32 bits of tracked range size
...
- **QUERY** (2): the driver writes (IOVA, size, page_size) into a
  ...
  ``DIRTY_CTRL=QUERY``. The device validates the requested range
  against enabled ranges (IOVA, size, page alignment), copies the
  dirty bitmap into the shared buffer, clears the tracked bits only
  after successful DMA, and sets the buffer's completion status. The
  driver should read ``DIRTY_STATUS`` after completion to check for
  errors.
...
device-written completion fields::
  ...
  0x4C    reserved[13]       -           Pad to 64-byte cache line
