...
- **DIRTY_ENABLE** (4): the driver fills an ``igb_mig_dirty_enable_req``
  ...
  range and begins recording pages touched by DMA. The page granularity
  is fixed at 4096. The driver checks ``STATUS`` for errors after the
  command completes.
...
``reserved`` must be zero::
  ...
  0x08    pgsize       uint64    Page granularity (must be 4096)
